...
from model.MyNet import FM,Decode
# from mmcv.cnn import ConvModule
from model.simple_diffusion import ResnetBlock
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.ca_attention == 1:
            ...
            s1=ss[:,:1,:]
            s=ss[:,1:,:]
            s=s.reshape(B, H, W, self.ca_num_heads, C//self.ca_num_heads).permute(3, 0, 4, 1, 2)
            for i in range(self.ca_num_heads):
            ...
            s_out = s_out.reshape(B, C, -1).permute(0, 2, 1)
            s_out=torch.cat((s1,s_out),1)
            x = s_out * v
            ...
            x = (attn @ v).transpose(1, 2).reshape(B, -1, C) + \
                self.local_conv(v.transpose(1, 2).reshape(B, -1, C).transpose(1, 2).view(B,C, H, W)).view(B, C, -1).transpose(1, 2)
            x=torch.cat((s1,x),1)
        x = self.proj(x)
        x = self.proj_drop(x)
    ...
    def __init__(self, head_conv, dim, mask_chans=1):
        ...
        if mask_chans != 0:
            self.mask_proj = nn.Sequential(
                nn.Conv2d(mask_chans, dim, kernel_size=head_conv, stride=2,padding=3 if head_conv==7 else 1, bias=False),
...
class SMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[2, 2, 8, 1], ca_attentions=[1, 1, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, mask_chans=1):
        super().__init__()
    ...
    def forward_features(self,g,timesteps,cond_img):
        B = g.shape[0]
        layer_features = []
        for i in range(self.num_stages):

            time_token = self.time_embed[i](timestep_embedding(timesteps, self.embed_dims[i]))
            time_token = time_token.unsqueeze(dim=1)
            patch_embed = getattr(self, f"patch_embed{i + 1}")
            block = getattr(self, f"block{i + 1}")
            norm = getattr(self, f"norm{i + 1}")
            if i==0:
                x, H, W = patch_embed(cond_img,g)
                x = torch.cat((time_token,x),1)
                for blk in block:
                    x = blk(x, H, W)
                x = norm(x)
...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        time_token = x[:, :1, :].reshape(B, 1, C)  # Fixme: Check Here
...
from model.Models_Train import ImageDepthNet
# from model.Models_Test import ImageDepthNet
class net(nn.Module):
    def __init__(self, class_num=1, mask_chans=0,trian=True, **kwargs):
        super(net, self).__init__()
        self.class_num = class_num
        self.backbone = smt_t(in_chans=3, mask_chans=mask_chans)

        self.decode_head = Decode(32, 32, 32, 32)
        self.fm_1 = FM(64)
        self.fm_2 = FM(128)
        self.fm_3 = FM(256)
        self.fm_4 = FM(512)
    ...
    def forward(self, x, timesteps, cond_img):
        ...
        for i in range(4):
            fuse = self.fms[i](features[i],timesteps)
            fuses.append(fuse)

        features, layer1, layer2, layer3 = self.decode_head(fuses[0],fuses[1],fuses[2],fuses[3],x,384)
        return features

    def _init_weights(self):
        model_path = './model/smt_tiny.pth'
        self.backbone.load_state_dict(torch.load(model_path)['model'], strict=False)
        print(f"loading pre_model ${model_path}")
...
if __name__ == '__main__':
    ...
    model = net()
    # model = model.cuda()
    # x=torch.rand(1, 1, 384, 384).cuda()
    # timesteps=torch.rand(1,384).cuda()
    # cond_img = torch.rand(1, 3, 384, 384).cuda()
    # output = model(input)
    # print(model)
