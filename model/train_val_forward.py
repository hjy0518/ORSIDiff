...
def modification_train_val_forward(model: nn.Module, gt=None, image=None, seg=None, **kwargs):
    ...
    if model.training:
        ...
        if time_ensemble:
            ...
            def process(i, p, gt_size):
                p = F.interpolate(p.unsqueeze(0), size=gt_sizes, mode='bilinear', align_corners=False)
                p = normalize_to_01(p)
                ps = F.interpolate(preds[i].unsqueeze(0), size=gt_sizes, mode='bilinear', align_corners=False)
                for i in range(len(ps[0])):
                    ps[:,i,:,:] = normalize_to_01(ps[:,i,:,:])
                preds_round = (ps > 0.5).float().mean(dim=1, keepdim=True)
                p_postion = (preds_round > 0.4).float()
                p = p_postion * p
                return p
            pred = [process(index, p, gt_size) for index, (p, gt_size) in enumerate(zip(pred, gt_sizes))]
...
def modification_train_val_forward_f(model: nn.Module, gt=None, image=None, seg=None, **kwargs):
    """This is for the modification task. When diffusion model add noise, will use seg instead of gt."""
    if model.training:
        assert gt is not None and image is not None and seg is not None
        return model(gt, image, seg=seg, **kwargs)
        ...
        if time_ensemble:
            ...
            def process(i, p, gt_size):
                ...
                return p
            pred = [process(index, p, gt_size) for index, (p, gt_size) in enumerate(zip(pred, gt_sizes))]
        # pred = F.interpolate(pred, size=gt_sizes, mode='bilinear', align_corners=False)
